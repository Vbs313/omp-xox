...
# Embedded file name: wfp_tests.py
...
from wfp_core import BLANK_LINE_MODE_DELETE_SINGLE, BLANK_LINE_MODE_KEEP_SINGLE, BLANK_LINE_MODE_PRESERVE, LegacyConversionUnavailable, WordProcessor

class TextNormalizationTests(unittest.TestCase):

    def test_symbol_normalization_keeps_decimal_numbers(self):
        self.assertEqual(WordProcessor._normalize_symbols_in_text("你好,世界."), "你好，世界。")
        self.assertEqual(WordProcessor._normalize_symbols_in_text("3.14 是 pi"), "3.14 是 pi")

    def test_ellipsis_and_quotes(self):
        self.assertEqual(WordProcessor._normalize_symbols_in_text('他说"你好"...'), "他说“你好”……")
        self.assertEqual(WordProcessor._normalize_symbols_in_text("version 1.2"), "version 1.2")
...
class BlankLineTests(unittest.TestCase):
    ...
    def test_delete_single_blank_line_and_compress_multiple(self):
        text = "a\n\nb\n\n\nc"
        self.assertEqual(WordProcessor._remove_blank_lines_from_text(text), "a\nb\n\nc")

    def test_keep_single_blank_line_and_compress_multiple(self):
        text = "a\n\nb\n\n\nc"
        self.assertEqual(WordProcessor._remove_blank_lines_from_text(text, keep_single_blank_lines=True), "a\n\nb\n\nc")

    def test_blank_line_mode_aliases(self):
        self.assertEqual(WordProcessor._normalize_blank_line_mode("preserve"), BLANK_LINE_MODE_PRESERVE)
        self.assertEqual(WordProcessor._normalize_blank_line_mode("delete_single"), BLANK_LINE_MODE_DELETE_SINGLE)
        self.assertEqual(WordProcessor._normalize_blank_line_mode("keep_single"), BLANK_LINE_MODE_KEEP_SINGLE)
...
class TempAndConversionTests(unittest.TestCase):
    ...
    def test_temp_docx_path_uses_system_temp_and_safe_name(self):
        ...
        try:
            self.assertEqual(os.path.normcase(os.path.abspath(os.path.dirname(temp_path))), os.path.normcase(os.path.abspath(tempfile.gettempdir())))
            self.assertRegex(os.path.basename(temp_path), "^~temp_copy_bad_name_[0-9]+_[0-9a-f]{8}\\.docx$")
    ...
    def test_txt_conversion_uses_blank_line_mode_and_cleans_temp_file(self):
        with tempfile.TemporaryDirectory(prefix="wfp273_test_") as tmpdir:
            ...
            try:
                ...
                self.assertEqual(os.path.normcase(os.path.abspath(os.path.dirname(temp_docx))), os.path.normcase(os.path.abspath(tempfile.gettempdir())))
                self.assertFalse(list(Path(tmpdir).glob("~temp_*.docx")))
                converted = Document(temp_docx)
                self.assertEqual([p.text for p in converted.paragraphs], [
                 "标题", "正文一", "", "正文二"])
            finally:
                processor._cleanup_temp_files()
...
# okay decompiling /tmp/【微信公众号：刺客边风】 Word-Formatter-Pro.v2.7.4.exe_extracted/wfp_tests.pyc
